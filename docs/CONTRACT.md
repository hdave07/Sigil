...
4. ~~Mission-setup screen: rebuild as a checklist~~ **Built.** Checklist (allow/requireApproval per action type) + off-mission trip-word tag input, submitting to `POST /agents/:id/mission`. Note this also settled a related question: the dashboard **cannot** create new agents (an agent's identity is its key pair, which only the agent itself ever holds) — the screen instead picks from agents that have already self-registered via their own `POST /agent` call and have no mission yet.
