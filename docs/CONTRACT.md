...
| GET | `/agents/:id/mission` | — | `Mission` |
| POST | `/missions/suggest-scope` | `{ text }` | `SuggestedMissionScope` |
...
2. ~~`allowedActions` ↔ `MissionScope` mapping isn't decided~~ **Resolved.** `GET /agents/:id/mission` (unsigned) now returns the full `Mission`, including `scope`. `Agent.allowedActions` can be populated from `mission.scope.allow` (or `allow.concat(requireApproval)` to show everything the agent could ever attempt) — which combination to show is still the dashboard's own display call, just no longer blocked on missing data.
...
- **2026-07-24** — Added `GET /agents/:id/mission` (unsigned) so the dashboard can read back an agent's full `Mission` (including `scope`) — resolves §5 item 2 (`allowedActions` mapping was blocked on this). LK built the mission-setup checklist screen against `POST /agents/:id/mission`.
- **2026-07-26** — Added `POST /missions/suggest-scope` (unsigned, dashboard-facing, same trust tier as `POST /agents/:id/mission`) to propose a per-mission action-type vocabulary (`allow`/`requireApproval`, each with a human label + one-sentence reason) plus `offMissionKeywords`, for the Setup screen's checklist to pre-populate as the human types. This is exactly what §7 already anticipated: "The right place for an LLM, if/when one is added, is authoring, not enforcement... helping a human draft allow/requireApproval/offMissionKeywords from plain-language mission text, with the human reviewing and confirming the generated scope before it's locked in." `checkAction()` is unchanged and never sees this endpoint; the suggestion writes no audit entry — only the human's eventual `POST /agents/:id/mission` submission does. Vocabulary is intentionally not constrained to the static 9 `ACTION_TYPES` — `checkAction()` already treats `action.type` as an opaque string, so the model can propose new mission-specific types (e.g. `payment.refund`), which the dashboard shows as extra rows alongside the static baseline. `SuggestedMissionScope` is a strict superset of `MissionScope` (`{type,label,reason}` items instead of bare strings) — `contract.ts`'s `MissionScope` itself is unchanged. New backend module `sigil-backend/packages/middleware/src/missionScopeSuggester.ts`.
  - **Provider: Groq (free tier), model `llama-3.3-70b-versatile`, via the `openai` SDK pointed at Groq's OpenAI-compatible endpoint** (`https://api.groq.com/openai/v1`) — chosen over Anthropic for this feature specifically to keep the demo at $0 cost. `GROQ_API_KEY` loaded from a new gitignored `sigil-backend/.env.local` (see `.env.local.example`) — the first `.env.local` convention in the backend package, matching what the dashboard already does; set the real key on Railway as a normal environment variable in production. Free-tier limits on this model: 30 req/min, 1,000 req/day, 12,000 tokens/min, 100,000 tokens/day (per-organization, not per-key) — tight for repeated demo run-throughs in one day, comfortable for a single run.
  - Llama 3.3 70B is not on Groq's strict-schema allowlist (only their `gpt-oss` models support constrained decoding), so unlike Anthropic's `output_config.format` this response is **not schema-guaranteed** — `missionScopeSuggester.ts` uses basic JSON mode (`response_format: {type: "json_object"}`) plus an explicit JSON shape spelled out in the system prompt, a code-fence-stripping step, and a runtime `isValidSuggestion()` shape check before ever returning a result; a malformed response throws, which `server.ts`'s existing catch-all still maps to a clean 502.
  - Swapping providers again only touches this one file — `server.ts`'s endpoint, the `SuggestedMissionScope`/`SuggestedActionType` shapes, and 100% of the frontend (`lib/api.ts`'s `suggestMissionScope()`, `app/setup/page.tsx`) are provider-agnostic and unchanged.
