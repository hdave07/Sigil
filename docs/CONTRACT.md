...
4. **Mission-setup screen: rebuild as a checklist, not free-text parsing.** The plain-language text box stays (still saved and displayed as `Mission.text`), but it doesn't drive enforcement — instead, build a checklist for which action types are auto-allowed (`MissionScope.allow`) vs. always-need-approval (`MissionScope.requireApproval`), plus a plain text/tag input for off-mission trip words (`MissionScope.offMissionKeywords`, direct substring match, not derived from the description). Submit the whole thing to the new `POST /agents/:id/mission` (unsigned, no signing code needed on the dashboard side at all).
